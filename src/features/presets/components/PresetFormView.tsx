...
export function PresetFormView() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Create Preset</h1>
          <p className="text-muted-foreground text-sm">Define a reusable creative direction</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <Card className="bg-card border-border shadow-sm">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Minimal Neutral Wedding" required className="bg-card border-border text-foreground" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/90">Short Description</Label>
              <Input value={shortDescription} onChange={(e) => setShortDescription(e.target.value)} placeholder="Brief summary of this style direction" className="bg-card border-border text-foreground" />
            </div>
            <div className="space-y-2">
        ...
        <Card className="bg-card border-border shadow-sm">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea value={systemPrompt} onChange={(e) => setSystemPrompt(e.target.value)} placeholder="Describe the style, color palette, typography, composition, mood, and any specific design rules..." required rows={6} className="bg-card border-border text-foreground placeholder:text-muted-foreground resize-none" />
              <p className="text-xs text-muted-foreground">This prompt guides AI generation. Be specific about colors, typography, mood, and decorative elements.</p>
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/90">Style Tags</Label>
              <div className="flex gap-2">
                <Input value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addTag())} placeholder="Add a tag and press Enter" className="bg-card border-border text-foreground" />
              ...
              {styleTags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {styleTags.map((tag, i) => (
                    <Badge key={i} variant="outline" className="border-violet-200 dark:border-violet-500/20 text-violet-700 dark:text-violet-400 bg-violet-50 dark:bg-violet-500/10 gap-1">
                      {tag}
                      <button type="button" onClick={() => setStyleTags(styleTags.filter((_, idx) => idx !== i))} className="cursor-pointer"><X className="w-3 h-3" /></button>
                    </Badge>
                  ))}
                </div>
              )}
        ...
        <Button type="submit" disabled={saving || !name || !systemPrompt} className="w-full h-12 bg-violet-600 hover:bg-violet-700 text-white font-semibold shadow-lg shadow-violet-200 gap-2 cursor-pointer">
          {saving ? <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving...</> : <><Save className="w-5 h-5" />Save Preset</>}
        </Button>
      </form>
    </div>
  );
